...
{{--    </div>--}}
    ...
    <div class="col-md-12 mt-2 row">
        <div class="form-group mg-b-0">
            ...
            @php
                $attachedFiles = (isset($campaign->attached_files) && is_array($campaign->attached_files))?$campaign->attached_files:[];
                $newAttachedFiles = [];
                foreach ($attachedFiles as $attachedRow){
                    if(isset($attachedRow['id'])){
                       $newAttachedFiles[$attachedRow['id']] = $attachedRow['name'];
                    }
                }
            @endphp
            @include('admin.dashboard.layouts.includes.upload-file-input', ['inputName' => 'attached_files[]', 'inputId' => 'attached_files', 'oldValues' => $newAttachedFiles, 'uploadPath' => url('photos/campaign').'/', 'disabledJs' => true, 'disabledCss' => true])
        </div>
    </div>

    <div class="col-12">
        <div class="grand-save-stb">
            <button type="button" class="btn btn-primary previous-btn-step btn-lg pull-right" data-step="2">Previous</button>
            <button type="button" class="btn btn-primary next-btn-step btn-lg pull-right" data-step="2">Save Campaign</button>
        </div>
    </div>
</div>
