<script>
    ...
    function swalDel(id, tabel ){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed){
                let reqUrl = ``;
                if(typeof id == "string")
                    reqUrl = `/dashboard/jobs/${id}`;
                else if(typeof id == "object")
                    reqUrl = `/dashboard/jobs-delete`;
    ...
    $(document).ready( function () {
        ...
        let jobsTable = $("#jobsTable").DataTable({
            ...
            'aaSorting': [[1, 'asc']],
            "ajax": {
                url: "{{route('dashboard.jobs.datatable')}}",
            ...
            "columns": [
                ...
                },
                {
                    "data": "id",
                    render: function (data, type) {
                        let route_edit = `/dashboard/jobs/${data}/edit`
                        return `<td>
                         @can('update jobs')
        ...
        $('#jobsTable tbody').on( 'change', '.switch_toggle', function (event) {
            let id = $(this).data('id');
            $.ajax({
                url:`/dashboard/jobs-toggle-status/${id}`,
                type:'POST',
                headers:{'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')},
                success:(response)=>{
