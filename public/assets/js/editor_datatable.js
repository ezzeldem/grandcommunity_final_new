...
var datetime = $.extend(true, {}, baseFieldType, {
    create: function (conf) {
        // let visitDateDateDiv = document.getElementById("campVisitDateData")
        // let deliverDateDateDiv = document.getElementById("campDeliverDateData")
        //
        // let visitDateDateArr = visitDateDateDiv ? document.getElementById("campVisitDateData").innerText.split(" - ") : []
        // let deliverDateDateArr = deliverDateDateDiv ? document.getElementById("campDeliverDateData").innerText.split(" - ") : []
        //
        // let visitDateStartDate = visitDateDateArr.length ? getDate(visitDateDateArr[0]) : ''
        // let visitDateEndDate = visitDateDateArr.length ? getDate(visitDateDateArr[1]) : ''
        // let deliverDateStartDate = deliverDateDateArr.length ? getDate(deliverDateDateArr[0]) : ''
        // let deliverDateEndDate = deliverDateDateArr.length ? getDate(deliverDateDateArr[1]) : ''

        let visitDateStartDate = $('#campVisitDateData').attr("attr-visitStartDate") ? $('#campVisitDateData').attr("attr-visitStartDate") : '';
        let visitDateEndDate = $('#campVisitDateData').attr("attr-visitEndDate") ? $('#campVisitDateData').attr("attr-visitEndDate") : '';
        let deliverDateStartDate = $('#campDeliverDateData').attr("attr-deliverStartDate") ? $('#campDeliverDateData').attr("attr-deliverStartDate") : '';
        let deliverDateEndDate = $('#campDeliverDateData').attr("attr-deliverEndDate") ? $('#campDeliverDateData').attr("attr-deliverEndDate") : '';

        let datesArr = [];
        datesArr.push(new Date(visitDateStartDate ? visitDateStartDate : deliverDateStartDate))
        datesArr.push(new Date(deliverDateStartDate ? deliverDateStartDate : visitDateStartDate))
        datesArr.push(new Date(visitDateEndDate ? visitDateEndDate : deliverDateEndDate))
        datesArr.push(new Date(deliverDateEndDate ? deliverDateEndDate : visitDateEndDate))

        let minDate = new Date(Math.min(...datesArr));
        let maxDate = new Date(Math.max(...datesArr));
